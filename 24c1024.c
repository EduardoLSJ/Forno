...
void I2C_repeatedstart(void) // assumes SDA and SCL are low at entry
{
    I2C_SDA1 = 1;
    I2C_SDATRIS = 0;
    I2C_delay();
...
void I2C_init() { //SDA e SCL high at entry
    unsigned char x;
    I2C_SDA1 = 1;
    I2C_SDATRIS = 1;
    for (x = 9; x != 0; x--) {
...
bit I2C_write(unsigned char adr) {
    ...
    I2C_SDATRIS = 1; // SDA goes into input-mode
    ...
    I2C_SDA1 = 0;
    I2C_SDATRIS = 0;
    if (result == 0) error_24c1024 = 1;
    return result;
}

int is_24c1024_present(void) {//ACKNOWLEDGE POLLING
    static bit result;
    I2C_start();
...
void wait_24c1024_done(void) {//ACKNOWLEDGE POLLING
    static bit done = 0;
    unsigned char x = 0;
    error_24c1024 = 0;
    for (x = 0; x < 125; x++) {
    ...
    if (done == 0) error_24c1024 = 1;
}

unsigned char read_byte_24c1024(unsigned int adr) {//Random Read
    unsigned char result;
    wait_24c1024_done(); // wait for the completion of a possible writeoperation
    I2C_start();
...
unsigned char read_next_byte_24c1024(void) {//Current Address Read
    unsigned char result;
    I2C_start();
    I2C_write(0xa1);
    result = I2C_read(0);
...
void read_buffer_24c1024(unsigned int adr, unsigned char* dat,
        unsigned char count) { //Sequential Read
    if (count == 1) {
        *dat = read_byte_24c1024(adr);
        return;
    }
    wait_24c1024_done(); // wait for the completion of a possible writeoperation
    I2C_start();
...
void write_byte_24c1024(unsigned int adr, unsigned char dat) {//Byte Write
    wait_24c1024_done(); // wait for the completion of a possible writeoperation
    I2C_start();
